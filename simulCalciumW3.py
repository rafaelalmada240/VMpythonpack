...
#####################################################################################################################
    ...
    Ldist = np.linalg.norm(coords,axis=1)
    
    for lr in L_List:
        # Run simulations
        for gw in Lw_List:
            par = [0.3,0.1,1, gw, 0.1]
            Lr = lr*K_run*np.median(av)**(3/2)
            Lw = (np.sqrt(woundsize)*0-lr)*K_run*np.median(av)**(3/2)
            print(par)
            i = 0
            transitionsList = []
            periWList = []
            ...
            gamma_vec[:,0] = gamma_vec[:,0]+gw*np.array(Ldist>4)*np.array(Ldist<5)
            gamma_vec[:,1] = gamma_vec[:,1]+np.array(Ldist>4)*np.array(Ldist<5)
            gamma_vec[:,2] = gamma_vec[:,2]+dt/10*np.array(Ldist>4)*np.array(Ldist<5)
            ...
            while (i < M) and ((areaWound>= areaWound0/8) and (areaWound<= 8*areaWound0)):
                ...
                G_vec = []#(G_run+copy.deepcopy(gamma_vec[:,0]))*(np.ones(coords.shape[0]))*np.median(av)
                for n in range(len(av)):
                    G_vec.append(G_run + gamma_vec[n,0]*np.median(av))
                ...
                #Noise
                J = 0.
                Rand_vertex = J*(np.random.rand(vertex_evo.shape[0],2)-1/2)
                Rand_vertex[:,0] = Rand_vertex[:,0]*(np.linalg.norm(vertex_evo,axis=1)>3)*(np.linalg.norm(vertex_evo,axis=1)<5)
                Rand_vertex[:,1] = Rand_vertex[:,1]*(np.linalg.norm(vertex_evo,axis=1)>3)*(np.linalg.norm(vertex_evo,axis=1)<5)
                ...
                if UseT1:
                    edges, vertex_evo, regions, transition_counter =  tpt.T1transition2(np.array(vertex_evo),np.array(edges),regions, pointregion,0.05*r0)
                ...
                strain = -(np.array(A0_run) - np.array(av1))/(np.array(A0_run)+1*(np.array(A0_run)==0))
                ...
                dgdt = cd2.g_up_tissue2(strain,pointregion,gamma_vec,par,wloc)
                ...
                if (i%(M//20) == 0) and list_plw != 1:
                    ...
                    print('max gamma - '+str(np.max(G_vec[:]))+','+str(np.mean(G_vec[:])))
                    print('max/mean force - '+str(np.max(np.linalg.norm(F_vertex,axis=1)))+';'+str(np.mean(np.linalg.norm(F_vertex,axis=1))))
                i = i + 1
                ...
                list_coords.append(copy.deepcopy(coords_evo))
                list_vertex.append(copy.deepcopy(vertex_evo))
                ...
                with open(final_directory+'/strain'+str(i)+'.txt','a') as ctxt:
                    for alpha in pointregion:
                        ctxt.write(str(alpha)+';'+str(strain[alpha])+'\n')
