...
def open_file(foldername0,N,issubfolder):
    ...
    for i in range(N):
        ...
        with open(foldername+'edges'+str(i)+'.txt','r') as text:
            for line in text:
                last_line = (line.replace("\n","")).split(';')
                l4 = re.split(' |, ',last_line[1].replace("[","").replace("]",""))
...
def saveframe(x_array, v_array, ridge_array_1, boundaries, N_init,N_fin,f_step,file_path,regions, wloc):
    ...
    for k in range(N_init,N_fin,f_step):
        ...
        ordered_boundary = sppv.rearrange(len(regions[k][wloc]),sppv.adj_mat(regions[k][wloc],ridge_array))
        ordered_boundary.append(ordered_boundary[0])
        for i in range(len(ridge_array)):
           
            if any(np.array(ridge_array[i])==-1)==0:
                plt.plot(v_array[k,ridge_array[i],0],v_array[k,ridge_array[i],1],'-',color='black',alpha=0.75,lw=2)
        ...
        for r in range(len(regions[k])):
            if r != wloc:
                region = list(np.array(regions[k][r])[sppv.rearrange(len(regions[k][r]),sppv.adj_mat(regions[k][r],ridge_array))])
                region.append(region[0])
                if (not -1 in region):
                    polygon = [v_array[k,j] for j in region]
                    if len(polygon) <= 4:
                        plt.fill(*zip(*polygon),'darkviolet',alpha=0.50)
                    if len(polygon) == 5:
                        plt.fill(*zip(*polygon),'midnightblue',alpha=.50)
                    if len(polygon) == 6:
                        plt.fill(*zip(*polygon),'deepskyblue',alpha=.50)
                    if len(polygon) == 7:
                        plt.fill(*zip(*polygon),'green',alpha=.50)
                    if len(polygon) >= 8:
                        plt.fill(*zip(*polygon),'darkorange',alpha=.50)
        
        wound_poly =  [v_array[k,i] for i in list(np.array(boundaries[k][1])[ordered_boundary]) if fc.norm(v_array[k,i]) < np.sqrt(2)*5]    
        plt.fill(*zip(*wound_poly),'r',alpha=.50)
