...
def T1transition2(vertices, ridges, regions, point_region,thresh_len):
    ...
    for i in vertex_list:
        ...
        # Two cells adjacent to (i,v_min) - 
        # Discrete Poincaré dual operation on a cellular complex 
        # (Correspondence between primal graph (Vertices) and dual graph (Cells) - Finding the dual edge)
        ...
        if lv_min <= thresh_len: 
            ...
            #Region reassignment - Pachner (2-2) move on the dual complex - 
            # swaps cells in a quadrilateral formed in the dual (Delaunay) triangulation
            ...
            # Finds Steiner points - vertices where three cells meet after changes
            ...
            # Doing the rotations before the edge change does not work adequately since we need to know what are the new neighbourhoods
            #, so instead, we will make the energy-based rotation after changing the edges
